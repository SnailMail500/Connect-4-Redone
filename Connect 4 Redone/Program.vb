...
Module Program
    ...
    Function checkDiagonalLeft(ByVal symbol) 'Instead of using symbols here i could and maybe should have passed through a player number and got it to check, or maybe i could have added an extra if statement (but wouldn't that be repeating code which is... bad practice?)
        For i As Integer = 1 To 4
            For j As Integer = 4 To 6 'I think this works ish
                If i < 4 And j < 3 And gameBoard(i, j) = symbol And gameBoard(i + 1, j + 1) = symbol And gameBoard(i + 2, j + 2) = symbol And gameBoard(i + 3, j + 3) = symbol Then
                    Return True
                ElseIf i >= 4 Or j <= 3 Then
                    Return False
                Else
                    Return False
                End If
    ...
    'Console.WriteLine("Coming soon, after I'm not finding out how to do file handling from some obnoxious stackoverflow post which makes no sense")
    'End Sub
    'NOTE: this was a failed experiment at file handling- there are subs above to prove that I'd met the objective that requires the subs to be set up!
End Module
'Just a quick note that I broke this program about 2 hours of tinkering on the day it was due and it was only rescuable because of an old git commit I did. Git ftw, if I used OneDrive it would have been unfixable :/